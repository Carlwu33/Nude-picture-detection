...
# 新建列表new_detected_regions, 其元素将是包含一些代表像素 Skin对象的列表
# new_detected_regions的元素即代表皮肤区域，元素索引为区域号
        new_detected_regions = []

# 将 merge_regions中的元素中的区域号代表的所有区域合并
...
# Self._clear_regions()方法只将像素大于指定数量的皮肤区域保留到self.skin_regions
...
# self.analyze_regions()的工作只是进行一系列的判断，得出图片是否色情的结论
    def _analuse_regions(self):
# 如果皮肤区域小于3个，不是色情
...
# 如果皮肤其区域与整个图像的比值小于 15%，那么结论就不是色情图片
...
if __name__ == "__main__":
    ...
    parse.add_argument('-r', '--resize', action='store_true',
                       help='Reduce image size to insrease speed of scanning')
    parse.add_argument('-v', '--visualization', action= 'store_true',
